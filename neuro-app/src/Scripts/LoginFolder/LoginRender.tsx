...
function LoginRender() {
  ...
  ///Formul de Recovery
  const [recoveryValues, setRecoveryValues] =
    useState<RecoveryEmail>(initialEmail);
        ...
        const getEmailData = async () => {
          try {
            const response = await fetch(
              `http://localhost:8192/users/mail/${formValues.emailFaculty}`,
              {
                method: "GET",
                headers: {
                  Authorization: `Bearer ${localStorage.getItem("token")}`,
            ...
            if (data.role == 0) window.location.href = "/Admin";
            else if (data.role == 1) window.location.href = "/Professor";
            else if (data.role == 2) window.location.href = "/Student";
          } catch (error) {
            console.error(error);
          }
        };
  ...
  const handleSubmit2 = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setRecoveryValues((prevRecoveryValues: RecoveryEmail) => ({
      ...prevRecoveryValues,
      submitted: true,
    }));
  ...
  const handleSubmit3 = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setRecoverCode((prevRecoveryValues: ForCode) => ({
      ...prevRecoveryValues,
      submitted: true,
    }));
  ...
  const handleSubmit4 = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setUpdateValues((prevRecoveryValues: ForUpdate) => ({
      ...prevRecoveryValues,
      submitted: true,
    }));
        ...
        <div className="row">
          <div className="left">
            <div className="text">
              ...
              <span>
                A platform that provides you a smart learning environment with
                support for courses, quizzes and testing.
              </span>
            </div>
          </div>
                    ...
                    <form onSubmit={handleSubmit2}>
                      <div className="input-field1">
                        <input
                          ...
                          id="email"
                          name="emailFaculty"
                          value={recoveryValues.emailFaculty}
